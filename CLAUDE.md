...
Web access token refresh: `SteamService.refreshWebAccessToken()` (companion) exchanges the stored long-lived `PrefManager.refreshToken` for a fresh short-lived `steamLoginSecure` access token via `steamClient.authentication.generateAccessTokenForApp(steamId, refreshToken, allowRenewal=false)` (the JavaSteam SampleWebCookie flow). The access token is only ~24h-lived, so any authenticated Steam *web* (store/community, e.g. curator endpoints in `SteamCuratorFetcher.kt`) request must refresh first. It persists both the new access token and any rotated refresh token back to `PrefManager`, and returns null (callers fall back to the cached token) when there's no connected client / steamID / refresh token.
