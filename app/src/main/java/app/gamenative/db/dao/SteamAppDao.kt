...
// SQL text. IN-list params (types/hiddenIds/categoryIds/favIds) are expanded to N placeholders;
// callers pass a [-1] sentinel (never a real app id) instead of an empty list so we never emit
// `IN ()`. [types] must be non-empty (the caller returns early otherwise).
//
